...
export async function POST(request){
    try {
        ...
        await connectDB();
        const insert=await SipCop.insertMany(add.map(item => ({IdVehiculo:item._id,Numero:item.Numero,IdPlaca:item.Placa,Marca:item.Marca,TipoVehiculo:item.Tipo.Tipo,IdTurno:turno._id.toString(),Turno:turno})));
