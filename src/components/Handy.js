...
export default function Handy({DataTurno}) {
    ...
    return <>
        ...
        {openModal.open&&<HandyModal Page={page}  openModal={openModal} fetchData={fetchData} DataTurno={DataTurno} setOpenModal={setOpenModal}/>}
    </>
}
...
function HandyModal({openModal,fetchData,setOpenModal,Page}){
    ...
    const fetchUpdate=(data)=>{
        if(!openModal.update){
            axios.post(`${process.env.API_URL}handy`, data).then(_=> {
            ...
            <Modal.Body>
                <form className="space-y-1" onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <Textarea name="Observacion" placeholder="Observación..." value={form.Observacion} onChange={handleChange}  required rows={4} />
                    </div>
                    <div>
                        <ToggleSwitch
                            checked={form.Activo}
